...
bernie$SUBJ = "Socialist"

#making a "typical" TD child
typical_td = data.frame(SUBJ = rep("Predicted TD", 6), VISIT = seq(1, 6), Diagnosis = "TD")

#making a "typical" ASD child
typical_asd = data.frame(SUBJ = rep("Predicted ASD", 6), VISIT = seq(1, 6), Diagnosis = "ASD")
...
td_group = dplyr::select(td_group, SUBJ, Diagnosis, VISIT, CHI_MLU)
...
visit = (seq(1:6))
avg_TD = data.frame(SUBJ="Average TD",VISIT = visit, Diagnosis = "TD", CHI_MLU = mlu)


#group of only asd kids
asd_group = subset(ken_train, Diagnosis == "ASD")
asd_group = dplyr::select(asd_group, SUBJ, Diagnosis, VISIT, CHI_MLU)

# real average of ASD children 
mlu = c(
...
visit = (seq(1:6))
avg_ASD = data.frame(SUBJ="Average ASD",VISIT = visit, Diagnosis = "ASD", CHI_MLU = mlu)


#all in one dataframe
plot2_data = merge(bernie, avg_TD, all = TRUE)
plot2_data = merge(plot2_data, avg_ASD, all = TRUE)
plot2_data = merge(plot2_data, typical_td,  all = TRUE)
plot2_data = merge(plot2_data, typical_asd,  all = TRUE)

# Getting absolute differences
round(abs(bernie$CHI_MLU - avg_TD$CHI_MLU),2)
round(abs(bernie$CHI_MLU - avg_ASD$CHI_MLU),2)
round(abs(bernie$CHI_MLU - typical_td$CHI_MLU),2)
round(abs(bernie$CHI_MLU - typical_asd$CHI_MLU),2)
